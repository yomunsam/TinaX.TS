...
namespace TinaX.TS
{
    [AddComponentMenu("TinaX/TypeScript/TS Behaviour")]
    public class TSBehaviour : XComponentScriptBase
    {
        /// <summary>
        /// 模块初始化方法
        /// </summary>
        /// <param name="behaviour">TsBehaviour 本体</param>
        /// <param name="uObjects">TsBehaviour 绑定对象</param>
        /// <param name="tDatas">TsBehaviour 绑定数据</param>
        public delegate void ModuleInit(TSBehaviour behaviour, UnityObjectBindingInfo[] uObjects, TypeBindingInfo[] tDatas);
        ...
        private ITypeScript m_TSManager;
        private ITSInternal m_TSManagerInternal;
        ...
        private void Awake()
        {
            if(this.ModuleName.IsNullOrEmpty() || this.ModuleName.IsNullOrWhiteSpace())
            {
                Debug.LogWarning($"[TSBehaviour] Invalid module name.");
                return;
            }
            //if (LuaScript == null)
            //    return;
